...
if [ ! -e "$oscheck"/gaster ]; then
    curl -k -sLO https://nightly.link/palera1n/gaster/workflows/makefile/main/gaster-"$oscheck".zip
...
"$oscheck"/iBoot64Patcher work/iBEC.dec work/iBEC.patched -b "-v rd=md0 debug=0x2014e wdt=-1 `if [ "$check" = '0x8960' ] || [ "$check" = '0x7000' ] || [ "$check" = '0x7001' ]; then echo "-restore"; fi`" -n
